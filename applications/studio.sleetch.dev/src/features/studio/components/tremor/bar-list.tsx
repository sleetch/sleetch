...
function BarListInner<T>(
  ...
  return (
    <div ref={forwardedRef} className={cn('flex justify-between space-x-6', className)} aria-sort={sortOrder} tremor-id="tremor-raw" {...props}>
          ...
          >
            <div
              className={cn(
                ...
                // background color
                'bg-primary',
                onValueChange ? 'group-hover:bg-primary/50' : '',
                // margin and duration
                {
      ...
      <div>
        {sortedData.map((item, index) => (
          <div
            key={item.key ?? item.name}
            className={cn('flex items-center justify-end', rowHeight, index === sortedData.length - 1 ? 'mb-0' : 'mb-1.5')}
          >
            <p
              className={cn(
                // base

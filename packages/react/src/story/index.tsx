...
>(data: {
	...
	Component: (props?: { variants?: (keyof V)[] }) => ReactElement;
} {
			...
			return (
				<div className={`no-sleetch-markdown ${styles.book}`}>
					{(selected ?? variants).map((variant) => (
						<div key={variant.toString()} className={`${styles.card} ${data.variants[variant].dotted ? styles.cardDotted : ''}`}>
							{render(variant)}
...
type StoryVariants<L> = L extends () => Promise<{ default: infer S }> ? (S extends { variants: readonly (infer V)[] } ? V : never) : never;

type BookProps<T extends Record<string, StoryLoader>, K extends keyof T> = {
	story: K;
	variants?: StoryVariants<T[K]>[];
};

export function defineBook<T extends Record<string, StoryLoader>>(stories: T) {
	function Component<K extends keyof T>({ story, variants }: BookProps<T, K>) {
